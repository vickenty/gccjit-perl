...
__PACKAGE__->generate_owned_class($owned_class_name, "gcc_jit_contextPtr", "context");
...
sub skip_xs_function {
    my ($self, $fn) = @_;
    return grep { $fn->{name} =~ $_ } @xs_blacklist;
}

sub init_package {
    my ($self, $package_name) = @_;
    return if $package_name !~ /^gcc_jit_.*Ptr$/ || $package_name eq "gcc_jit_contextPtr";
    no strict "refs";
    push @{"${package_name}::ISA"}, $owned_class_name;
}

sub parse_xs_name {
    my ($self, $name) = @_;
    return $name =~ /^gcc_jit_([a-z]+)_(.*)/;
}

sub package_for_class {
    my ($self, $class) = @_;
    return "gcc_jit_${class}Ptr";
}
...
__PACKAGE__->generate_from_xs("GCCJIT", q!
    void
